...
def getCharmanderLink(soup):
    for link in soup.find_all('a', attrs = {'href' : re.compile(r'/pokedex/charmander')}):
        final = ("https://pokemondb.net" + link.get('href'))
    return final
    #return ("https://pokemondb.net" + link.text)
...
def getEggMoves(pokemon):
    ...
    list = []
    count = 1
    for moves in soup.find('a', attrs = {'href' : re.compile(r'/move/')}):
        list.append(moves.text)
    return list
    #add code here
...
def findLetters(sentences):
    ...
    # find all the words that match the regular expression in each sentence
        words = re.findall(expr, sentence)   
